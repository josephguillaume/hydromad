...
plotPCNSE <- function(res, objectives = "r.squared", return.data = FALSE) {
    ...
    stopifnot(!"Catchment" %in% names(res) | 
                length(unique(res$Catchment)) == 1)
    resm <- melt(res, measure.vars = objectives,
                 id.vars = intersect(names(res), 
                                    c("Model.str", "Catchment", "calib.period",
                                       "sim.period", "Cal.objfn")),
    ...
    resm2 <- melt(resm2, id.vars = intersect(names(res2),
                                             c("Model.str", "Catchment", 
