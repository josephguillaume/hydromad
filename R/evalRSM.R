runRSM <- function(modx,...,objective = hydromad.getOption("objective")){
  ...
  coding <- lapply(names(bounds),
    function(var) {
      ...
      as.formula(sprintf("%s.c ~ (2*%s-(%f+%f))/(%f-%f)",
                         var, var, max.var, min.var, max.var, min.var))
    })
  
  evals <- rsm::ccd(as.formula(sprintf("~%s", 
                                       paste(sprintf("%s.c", names(bounds)), 
                                             collapse = "+"))),
               coding = coding,inscribed = TRUE, ...)

  cat(sprintf("Running %d model evaluations\n", nrow(evals)))
  evals$Y <- evalPars(decode.data(evals)[, names(bounds)], modx,
                      objective = objective)
