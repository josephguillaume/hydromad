...
fland1<-function(sma,fsum1)				
{
  ...
  if (abs(sma$uztwc) < EPS) sma$uztwc = 0
  e2 = 0
  if (sma$uztwc < 0) { 
    e1 = e1+sma$uztwc
    sma$uztwc = 0
  ...
  if (abs(sma$adimc) < EPS) sma$adimc = 0				
  if (sma$adimc < 0) { 
    e5 = e5+ sma$adimc
    sma$adimc = 0
  }
  ...
  if (ninc < sma$min_ninc) ninc=sma$min_ninc
  dinc = 1.0 /  ninc * sma$dt
  pinc = twx / ninc
  
  
  
  if (sma$uzk > 1) print(sma$uzk)
  if (sma$lzpk > 1) print(sma$lzpk)
  if (sma$lzsk > 1) print(sma$lzsk)
