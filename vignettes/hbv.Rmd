...
The required data in order to use the model are:

- P: timeseries of precipitation (mm)
- T: timeseries of average temperature (ºC)
- Q: timeseries of observed discharge (mm)

In addition potential evapotranspiration (PET) needs to be provided. There are three options:

- A timeseries of the same length as the main data
...
A vector of length 12 or 365 of mean temperature can also be provided for use in calculating PET. If it is not provided then mean monthly temperature is calculated from the timeseries of temperature.
...
A list with named objects `"PET"` and `"Tmean"` need to be supplied to the `PET` argument when setting up the model. In addition, the `cet` parameter needs to be provided. Either a single value or a range can be provided if `cet` is to be calibrated.
